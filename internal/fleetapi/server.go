...
// NewUplinkServer constructs a dedicated authenticated telemetry ingress. It
// deliberately has no dashboard, fleet-state, event, version, health, static,
// redirect, or catch-all route. This lets a reverse proxy publish the ingress
// listener without making any read-side Hub surface reachable.
func NewUplinkServer(source DataSource, config UplinkConfig) (*Server, error) {
	uplink, err := newUplinkHandler(source, config)
	if err != nil {
		return nil, err
	}
	server := &Server{source: source, uplink: uplink, uplinkOnly: true, mux: http.NewServeMux()}
...
func (s *Server) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	nonCanonicalPath := path.Clean(request.URL.Path) != request.URL.Path
	if s.uplinkOnly {
		nonCanonicalPath = nonCanonicalPath || request.URL.RawPath != "" || request.URL.RawQuery != "" || request.URL.ForceQuery
	}
	if (s.uplinkOnly || strings.HasPrefix(request.URL.Path, "/api/")) && nonCanonicalPath {
		writeError(writer, http.StatusNotFound, "API endpoint not found")
		return
	}

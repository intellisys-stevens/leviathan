...
func (a *application) uplinkCommand() *cobra.Command {
	var uplinkURL string
	var legacyHubURL string
	var instanceUUID string
	var tokenEnvironment string
	var pushInterval time.Duration
	command := &cobra.Command{
		Use:   "uplink",
		Short: "Continuously push local telemetry to an authenticated Yggdrasil ingress",
		Args:  cobra.NoArgs,
		RunE: func(command *cobra.Command, _ []string) error {
			if uplinkURL != "" && legacyHubURL != "" && uplinkURL != legacyHubURL {
				return errors.New("uplink URL was provided with conflicting values")
			}
			if uplinkURL == "" {
				uplinkURL = legacyHubURL
			}
			if uplinkURL == "" {
				return errors.New("uplink URL is required")
			}
			...
			client, err := uplinkclient.New(uplinkURL, token, uplinkclient.Options{})
			if err != nil {
				return err
			}
			if instanceUUID == "" {
	...
	command.Flags().StringVar(&uplinkURL, "uplink-url", "", "credential-free HTTPS Yggdrasil uplink origin")
	command.Flags().StringVar(&legacyHubURL, "hub-url", "", "deprecated alias for --uplink-url")
	...
	_ = command.Flags().MarkDeprecated("hub-url", "use --uplink-url")
	_ = command.Flags().MarkHidden("hub-url")
	return command
}

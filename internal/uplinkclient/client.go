...
// explicitly configured Yggdrasil ingress. It deliberately contains no retry,
// queue, server-side inventory, accounting, or credential-enrollment behavior.
...
// Envelope is the complete JSON body accepted by the compatibility uplink
// endpoint.
...
// New validates and retains one HTTPS Yggdrasil origin and bearer token.
// baseURL may contain only the HTTPS origin (an optional trailing slash is
// accepted).

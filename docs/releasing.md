...
4. Manually run CI on the resulting `main` commit and require that exact SHA and
   ...
   and publishes archives, SBOMs, the bridge image, and the Helm chart. Each
   native archive must include the `leviathan` and separate `leviathan-hub`
   binaries, the fleet OpenAPI contract, the Jetstream fleet guide, and the
   default-off `leviathan-uplink@.service` plus its non-secret environment
   example.
...
5. Verify the GitHub release checksums and attestations; both native archives;
   the version output from `leviathan` and `leviathan-hub`; the packaged
   `api/fleet-openapi.yaml`, `docs/jetstream-fleet.md`,
   `leviathan-uplink@.service`, and `leviathan-uplink.env.example`;

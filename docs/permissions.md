...
`leviathan uplink` is a separate, explicit network mode. It reads only the named
...
origin. The mode is default-off. Its token authenticates one Nova creator trust
domain rather than an individual VM, so it must remain limited to an explicitly
approved pilot and must not be used alone for security, billing, scheduling, or
incident attribution. Normal TUI, snapshot, watch, and serve commands retain
their existing outbound-network behavior.

...
CGO_ENABLED=0 "${go_command}" build \
  -trimpath -buildvcs=false \
  -ldflags "-s -w -X github.com/intellisys-stevens/leviathan/internal/hubcli.Version=${release_version} -X github.com/intellisys-stevens/leviathan/internal/hubcli.Commit=${commit} -X github.com/intellisys-stevens/leviathan/internal/hubcli.BuildDate=${build_date}" \
  -o "${stage}/leviathan-hub" ./cmd/leviathan-hub
...
cp contrib/systemd/leviathan.env.example contrib/systemd/leviathan-attribution.env contrib/systemd/leviathan-uplink.env.example "${stage}/"
cp contrib/systemd/leviathan-uplink@.service "${stage}/"
...
CGO_CFLAGS="${CGO_CFLAGS:--Wno-deprecated-declarations}" "${go_command}" run github.com/google/go-licenses/v2@v2.0.1 save ./cmd/leviathan ./cmd/leviathan-hub --save_path "${stage}/THIRD_PARTY_LICENSES/go" >&2

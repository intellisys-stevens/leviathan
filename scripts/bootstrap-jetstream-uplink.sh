...
uplink_hub_url=${LEVIATHAN_BOOTSTRAP_UPLINK_URL:-}
instance_uuid=
creator_username=
ssh_host=
ssh_user=exouser
...
The install command is read-only by default. It validates Hub policy, the SSH
...
Required install options:
  ...
  --creator-username USER       Exact creator label reported by the Hub
...
Optional install options:
  ...
  --verify-timeout SECONDS      Hub acknowledgement timeout (default: 90)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --state-url=*)
      ...
      ;;
    --uplink-hub-url)
      [[ $# -ge 2 ]] || fail "--uplink-hub-url requires a value"
      uplink_hub_url=$2
      shift 2
      ;;
    --uplink-hub-url=*)
...
[[ "${target_creator}" == "${creator_username}" ]] ||
  fail "creator mismatch between the selected Hub instance and --creator-username"
...
if [[ "${target_agent_source}" == leviathan_agent ]]; then
  fail "instance has an exact Leviathan pull binding; migrate that Hub binding before installing Uplink"
fi
...
# the mutating phase on a second, fresh Hub snapshot rather than the snapshot
...
while true; do
  ...
  [[ "${current_creator}" == "${creator_username}" ]] || fail "creator changed while waiting for Hub acknowledgement"
  [[ "${current_cloud}" == active && "${current_managed}" == true && "${current_eligible}" == true ]] ||
    fail "instance eligibility changed while waiting for Hub acknowledgement"
  if [[ "${current_status}" == available && "${current_source}" == leviathan_uplink ]]; then
    printf 'Leviathan Uplink is active and acknowledged by the Hub for %s (%s).\n' "${target_name}" "${instance_uuid}"
    exit 0
  fi
  if (( $(date +%s) >= deadline )); then
    fail "systemd is active but the Hub did not acknowledge Uplink within ${verify_timeout} seconds"
  fi
  sleep 5
done

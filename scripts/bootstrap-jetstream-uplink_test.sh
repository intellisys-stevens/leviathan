...
# Candidate listing uses only current Hub policy and makes no SSH connection.
: >"${curl_counter}"
...
# --apply installs, starts systemd, and waits for an exact Hub Uplink acknowledgement.
: >"${curl_counter}"
: >"${fake_log}"
...
assert_contains "${test_root}/apply-output" 'Leviathan Uplink is active and acknowledged by the Hub'

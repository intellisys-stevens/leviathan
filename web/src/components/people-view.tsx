...
function ResourceRow({
  ...
  const reserved = resource.state === 'reserved';
  ...
  const hardwareName = physical ? selection.gpu.name : selection.gi.profile;
  ...
  return (
    <li className="mobile-workload-resource interactive-resource flowing-surface group relative grid min-w-0 gap-3 rounded-lg px-3 py-3 sm:grid-cols-[minmax(0,1fr)_minmax(12rem,0.8fr)]">
      ...
      <div className="pointer-events-none relative z-0 flex min-w-0 items-start gap-2.5">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-1 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">
            {hardwareName ? (
              <span className="max-w-full break-words text-[13px] text-muted-foreground">
                {hardwareName}
              </span>
            ) : null}
            {!reserved ? <Badge variant="outline">Allocated</Badge> : null}
          </span>
        </span>
        <ChevronRight
      ...
      {reserved ? (
        <div className="pointer-events-none relative z-0 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          <Badge variant="outline">Reserved</Badge>
          <span>
            {formatBytes(memory.totalBytes)} capacity · activity not attributed
          </span>
        </div>
      ) : (
...
function PeopleViewComponent({
  ...
  if (
    ...
    snapshot.workloadTelemetry.status !== 'available'
  ) {
    return (
      <output
        className="block border border-dashed border-border bg-card p-6 text-sm text-muted-foreground"
        data-testid="workload-inventory-state"
      >
        Workspace inventory is{' '}
        {snapshot.workloadTelemetry.status === 'partial'
          ? 'incomplete'
          : 'unavailable'}
        .
        {snapshot.workloadTelemetry.message
          ? ` ${snapshot.workloadTelemetry.message}`
          : ''}
      </output>
    );
  }
  if (!attribution && !snapshot.workloadTelemetry && view.people.length === 0) {
    return (
      <section
      ...
      >
        ...
        <p className="mx-auto mt-1 max-w-lg text-[15px] text-muted-foreground">
          Hardware telemetry is in Resources.
        </p>
      </section>
    );
  }
  ...
  if (
    ...
    return (
      ...
      >
        Workspace assignments are {attribution.status}.
      </div>
    );
  }
    ...
    return (
      ...
      >
        {view.unresolvedAssignments > 0
          ? `${countLabel(view.unresolvedAssignments, 'assignment')} pending allocation verification.`
          : snapshot.workloadTelemetry?.status === 'unavailable' ||
              snapshot.workloadTelemetry?.status === 'stale'
  ...
  return (
    <div className="space-y-4" data-testid="people-view">
      {attribution && attribution.status !== 'available' ? (
        <output className="block border border-amber-500/25 bg-amber-500/[0.05] px-3 py-2 text-[13px] text-amber-700 dark:text-amber-300">
          GPU assignments are {attribution.status}.
        </output>
      ) : null}
      {snapshot.workloadTelemetry &&
      snapshot.workloadTelemetry.status !== 'available' ? (
        <output className="block px-3 py-2 text-[13px] text-muted-foreground">
          {snapshot.workloadTelemetry.status === 'partial'
            ? 'Partial host-resource telemetry.'
      ...
      {view.unresolvedAssignments > 0 ? (
        <p className="border border-amber-500/25 bg-amber-500/[0.05] px-3 py-2 text-[13px] text-amber-700 dark:text-amber-300">
          {countLabel(view.unresolvedAssignments, 'assignment')} pending
      ...
      >
        ...
        >
          <SnowCap surfaceKey="workload-owner-panel" />
          ...
          <div className="p-3 pb-0">
            ...
            >
              <WorkloadTelemetryChart
                ...
                ownerTelemetry={selectedPerson.telemetry}
          ...
          >
            {selectedPerson.workspaces.map((workspace) => (
              ...
              >
                ...
                {(workspace.unresolvedAssignments ?? 0) > 0 ? (
                  <output className="block px-3 py-2 text-[13px] text-muted-foreground">
                    {countLabel(
                      workspace.unresolvedAssignments ?? 0,
                      'allocation',
                    )}{' '}
                    ...
                    <li className="px-3 py-2 text-[13px] text-muted-foreground">
                      {attribution?.status === 'available'
                        ? attribution.resolution?.status === 'complete'
                          ? 'No assigned GPU resources.'
                          : 'GPU assignments pending verification.'
                        : 'GPU assignments unavailable.'}
                    </li>
                  ) : null}
